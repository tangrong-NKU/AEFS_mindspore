...
y_true = np.loadtxt('dataset/test_DPI.txt')
y_pre = np.loadtxt('results/y_pre_DPI.txt')
...
for i in tqdm(range(len(y_true))):
    if np.sum(y_true[i]) == 0:
        ...
        tpr1, fpr1, precision1, recall1 = tpr_fpr_precision_recall(y_true[i], y_pre[i])
...
np.savetxt('results/AEFS_fpr.txt', fpr_mean)
np.savetxt('results/AEFS_tpr.txt', tpr_mean)
np.savetxt('results/AEFS_recall.txt', recall_mean)
np.savetxt('results/AEFS_p.txt', precision_mean)
np.savetxt('results/AEFS_AUC_list.txt', auc_list)
np.savetxt('results/AEFS_AUPR_list.txt', aupr_list)